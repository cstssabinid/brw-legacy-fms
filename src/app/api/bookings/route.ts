...
function bookingSaveError(error: unknown) {
  ...
  if (code === "P1001" || message.includes("Can't reach database server")) {
    return { status: 503, error: "The booking system database is temporarily unavailable. Please contact Berwa Photo Hub on WhatsApp or email and include your booking details.", errorCode: "DATABASE_UNAVAILABLE" };
  }

  if (code === "P2021" || code === "P2022" || message.includes("does not exist in the current database") || message.includes("invalid input value for enum") || message.includes("Unknown argument")) {
    return { status: 500, error: "The booking system database needs to be updated before this booking can be saved. Please contact Berwa Photo Hub on WhatsApp or email while we resolve it.", errorCode: "DATABASE_SCHEMA_OUTDATED" };
  }

  if (message.includes("Unknown service") || message.includes("package")) {
    return { status: 400, error: message, errorCode: "BOOKING_SELECTION_UNAVAILABLE" };
  }

  return { status: 500, error: "Your booking could not be saved due to a system issue. Please try again or contact Berwa Photo Hub on WhatsApp or email.", errorCode: code || "BOOKING_SAVE_FAILED" };
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    try {
      ...
      if (admins.length > 0) {
        await prisma.notification.createMany({
          data: admins.map((admin) => ({
            ...
            message: `New ${result.service.name} booking received from ${result.client.fullName}. Reference: ${result.bookingReference}.`,
            link: "/admin/bookings"
          }))
        });
      }
      await prisma.auditLog.create({
        data: {
          ...
          entityType: "Booking",
          entityId: result.id,
          newValue: { bookingReference: result.bookingReference, service: result.service.name, client: result.client.fullName }
        }
      });
    } catch (notificationError) {
      console.error("Booking saved, but admin notification or audit log failed", notificationError);
    }
    ...
    return NextResponse.json({ error: response.error, errorCode: response.errorCode }, { status: response.status });
  }
}
